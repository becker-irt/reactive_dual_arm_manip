  ...
  for (int i = 0; i < obstacles.size() - 1; i++) {
    ...
    if (dist_obs < detect_shell_rad_) {
      if (!known_obstacles_.at(i)) {
        // For joining obstacles in the sim_dual_arm_static_3 scenario
        // I.e. same rotation vector for touching spheres
        // Eigen::Vector3d field_rotation_vec = calculateRotationVector(
        //     getLatestPosition(), getGoalPosition(), obstacles, i);
        // std::fill(known_obstacles_.begin(), known_obstacles_.end(), true);
        // std::fill(field_rotation_vecs_.begin(), field_rotation_vecs_.end(),
        //           field_rotation_vec);
        // For joining obstacles in the sim_dual_arm_static_2 scenario
        // if (i == 0) {
        //   known_obstacles_.at(0) = known_obstacles_.at(1);
        //   field_rotation_vecs_.at(0) = field_rotation_vecs_.at(1);
        // } else if (i == 2) {
        //   known_obstacles_.at(2) = known_obstacles_.at(3);
        //   field_rotation_vecs_.at(2) = field_rotation_vecs_.at(3);
        // } else {
